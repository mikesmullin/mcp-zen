...
export function createMcpServer(adapter) {
  const server = new Server({ name: "mcp-zen", version: "0.1.0" }, {
    capabilities: { tools: {} },
    instructions: "Agent-browser signatures on an attached Firefox/Zen browser, not a managed Chromium process. Default session binds a personal tab; named sessions use Firefox containers (permission required). Namespaces are logical container/session groups, not daemon filesystem sandboxes. Snapshots are DOM-derived accessibility approximations; input events are synthetic. Frames, find/hover, dialogs, tap/swipe are included. Unsupported launch/security/restore options fail explicitly. close only closes mcp-zen-owned tabs/containers, never the personal browser. Screenshot paths are on the MCP server host. See docs/parity.md. If EXTENSION_NOT_CONNECTED: ask Mike to load the temporary add-on (about:debugging → Load Temporary Add-on → firefox-extension/manifest.json). If AUTOMATION_PERMISSION_MISSING: ask him to Enable browser automation on the mcp-zen options page. Do not describe either as an empty page.",
  });
