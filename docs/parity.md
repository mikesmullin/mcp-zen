...
| `zen_browser_locate` / `zen_browser_reveal` / `zen_browser_click_at` / `zen_browser_set_range` | Additive. Locate never clicks. Reveal is synthetic hover. click_at uses relative x/y. set_range is native `<input type=range>` only. |
| `zen_browser_media_state` / `_seek` / `_play` / `_pause` | Additive, CSP-independent HTMLMediaElement inspection and verified seek/play/pause. Ambiguous or unverified operations error. |
