...
test("page runtime: snapshot/ref/action/wait workflows in a real DOM", { skip: !available && "Set CHROMIUM_PATH for DOM tests", timeout: 30000 }, async (t) => {
  ...
  const repeated = ok(await run("snapshot", { interactive: true, nextRef: snapshot.nextRef }));
  assert.deepEqual(repeated.refs, snapshot.refs, "unchanged nodes keep their refs");
  assert.equal(repeated.nextRef, snapshot.nextRef);
  const scoped = ok(await run("snapshot", { selector: "#duplicate-b" }));
  assert.deepEqual(Object.keys(scoped.refs), [duplicateRefs[1]], "scoping does not renumber refs");
  ...
  assert.equal((await run("click", { selector: "e99999" })).error.code, "STALE_REF");
  assert.equal((await run("click", { selector: duplicateRefs[1] }, { sessionId: "other" })).error.code, "STALE_REF");
  assert.equal((await run("click", { selector: duplicateRefs[1] }, { documentId: "old" })).error.code, "STALE_REF");
  assert.match((await run("snapshot", { selector: 'article:has-text("pstack")' })).error.message, /standard CSS.*default action is click/);
  ...
  await page.locator("#duplicate-a").evaluate((el) => el.replaceWith(el.cloneNode(true)));
  const replaced = ok(await run("snapshot", { selector: "#duplicate-a" }));
  assert.notEqual(Object.keys(replaced.refs)[0], duplicateRefs[0], "replacement nodes cannot inherit an old ref");
